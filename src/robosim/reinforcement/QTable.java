...
public class QTable {
    ...
    public int leastVisitedAction(int state) {
        int lowestAction = 0;
        int lowestVisits = visits[state][0];
        for (int i = 0; i < visits[state].length; i++) {
            if (visits[state][i] < lowestVisits) {
                lowestVisits = visits[state][i];
                lowestAction = i;
            }
        }
        return lowestAction;
    ...
    public int senseActLearn(int newState, double reward) {
        ...
        double newQ = ((1 - learningRate) * (getQ(lastState, lastAction)) + learningRate * (discount * getQ(newState, getBestAction(newState)) + reward));
